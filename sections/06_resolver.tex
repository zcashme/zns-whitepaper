...
The Resolver processes verified Name Notes in ascending canonical block order. Within a block, they are ordinarily processed according to transaction and action order, subject to the same-block release precedence rule below.
...
If the same block contains both one or more \action{update} Name Notes and one or more \action{release} Name Notes for the same name that reference the \field{rcm} of the same current accepted Name Note, the Resolver considers the \action{release} Name Notes before the competing \action{update} Name Notes regardless of their relative transaction order within that block.
