...
\draw[arrow] (wallet) -- node[label, above] {Request} (mint);
\draw[arrow] (mint) -- node[label, sloped, above] {Relay} (chain);
\draw[arrow] (chain) -- node[label, sloped, above] {Relay} (wallet);
\draw[arrow] (wallet) -- node[label, sloped, below] {Respond} (chain);
\draw[arrow] (chain) -- node[label, sloped, below] {Respond} (mint);
...
\node[font=\scriptsize, text=gray, below=0.15cm of mint] {treasury account};
